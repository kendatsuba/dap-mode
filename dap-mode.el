...
be used to compile the project, spin up docker, ...."
  ...
  (let* ((debug-args (dap-variables-expand-in-launch-configuration debug-args)))
    (if (string= (plist-get debug-args :type ) "compound")
        (mapcar (lambda (x) (dap-debug (cdr (assoc x (dap-launch-find-parse-launch-json)))))
                (plist-get debug-args :configurations))
